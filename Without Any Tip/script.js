...
document.getElementById("processBtn").addEventListener("click", () => {
  ...
  reader.onload = function (e) {
      ...
      for (const result of results) {
          if (result.params["Gd"] >= 0.5 || isNaN(result.params["B/A"])) {
              finalResult = result;
              break;
          }
      }
  ...
  function smoothElements(data) {
    return data.map((_, i) => {
      ...
      if (isExtreme) {
        ...
        const weightedAverage = (values[0] * weights[1] + values[1] * weights[0]) / (weights[0] + weights[1]);
...
let MOPD = null, HRD = null, MMD = null;

        function setMOPD() {
          MOPD = parseFloat(document.getElementById("MOPD").value);
            if (isNaN(MOPD)) {
                alert("Enter a valid number for Magma overpressure of dyke (PO)!");
                return;
            }
            document.getElementById("MOPDValue").innerText = MOPD;
        }

        function setHRD() {
          HRD = parseFloat(document.getElementById("HRD").value);
            if (isNaN(HRD)) {
                alert("Enter a valid number for Host Rock Density!");
                return;
            }
            document.getElementById("HRDValue").innerText = HRD;
        }

        function setMMD() {
          MMD = parseFloat(document.getElementById("MMD").value);
            if (isNaN(MMD)) {
                alert("Enter a valid number for Mafic Magma Density!");
                return;
            }
            document.getElementById("MMDValue").innerText = MMD;
        }

        function calculateDepth() {
            if (MOPD === null || HRD === null || MMD === null ) {
                alert("Please set all values before calculating!");
                return;
            }

            // Example calculation (Modify based on actual formula)
            const result = 1000000*(MOPD/9.8) / Math.abs(HRD - MMD);

            document.getElementById("resultValue1").innerText = result.toFixed(6);
        }